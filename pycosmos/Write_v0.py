...
# Read MiniSeed file 
   ...
   duration = mseed[0].stats.endtime - mseed[0].stats.starttime
   max_acc = max(mseed[chn].data)
...
# Parameters of Integer Header
   int_params = [0, 1, 51, 120, 1, -999, -999, 0, -999, -999, # 1-10
...
# Write Text Header to text file
   with open(fn, "w") as f:
    print("Raw acceleration counts   (Format v01.20 with 13 text lines)"
...
# Write acceleration data into file.
   data_array = mseed[chn].data
   data_size = data_array.size
   k = 0
   j = 0
   with open (fn, "a") as f:
    print("  ", data_size, "raw accel.   pts approx  ",int(duration), "secs, units=volts (51),Format=(8I10)", file = f)
    for i in range(0,data_size):
     try:
      if j == 0:     
       print(f"  {data_array[i]:8}", end=" ", file =f)
       j += 1
      else:
       print(f" {data_array[i]:8}", end=" ", file =f)
       j += 1
      ...
      break
   with open(fn, "a") as f:  
    print('\nEnd-of-data for',evt_chan,'  1', 'acceleration', file = f)
